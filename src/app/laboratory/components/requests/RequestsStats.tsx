...
export const RequestsStats = ({ stats, useCyanTheme, showCompletedAndDelivered = true }: RequestsStatsProps) => {
      ...
      <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${useCyanTheme ? 'bg-cyan-100' : 'bg-purple-100'}`}>
            <FileImage className={`w-5 h-5 ${useCyanTheme ? 'text-cyan-600' : 'text-purple-600'}`} />
          </div>
          <div>
